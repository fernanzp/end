...
<script>
    ...
    document.getElementById('continuar-btn').addEventListener('click', function() {
        ...
        // Mostrar la cantidad en la consola (para verificar que funciona)
        console.log('Cantidad donada:', cantidadDonate);
    });
</script>
...
<!-- Modal -->
<div id="paymentModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center" onclick="closeModalOnOutsideClick(event)">
    <div class="bg-white p-8 rounded-lg max-w-md w-full z-50" onclick="event.stopPropagation()">
        ...
        <!-- Métodos de pago -->
        <div class="mb-4">
            <div id="paypal-button-container" style="margin-top: 20px;"></div> <!-- Contenedor de PayPal --> 
        </div>
        <button onclick="closeModal()" class="mt-4 px-6 py-4 bg-red-600 text-white font-bold rounded hover:bg-red-800 w-full">
            Cancelar
        </button>
    </div>
</div>